...
const EmissionCalculator = () => {
  ...
  const calculateEmissions = () => {
    ...
    const transportEmission = parseFloat(formData.distance || 0) * transportEmissionFactor;
    ...
    const electricityEmission = parseFloat(formData.electricity || 0) * electricityFactor;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-green-50 to-white p-6">
      <h1 className="text-3xl font-bold text-gray-900 mb-6 text-center">Advanced Carbon Emission Calculator</h1>
      ...
      <div className="max-w-xl mx-auto bg-white p-6 rounded-xl shadow-md space-y-4">
        ...
        <div>
          <label className="block font-semibold text-gray-700">Distance (km):</label>
          <input type="number" name="distance" value={formData.distance} onChange={handleChange} className="w-full border p-2 rounded" />
        </div>

        {/* Electricity */}
        <div>
          <label className="block font-semibold text-gray-700">Electricity Use Type:</label>
          <select name="electricityType" value={formData.electricityType} onChange={handleChange} className="w-full border p-2 rounded">
        ...
        <div>
          <label className="block font-semibold text-gray-700">Electricity Usage (kWh):</label>
          <input type="number" name="electricity" value={formData.electricity} onChange={handleChange} className="w-full border p-2 rounded" />
        </div>

        {/* Food */}
        <div>
          <label className="block font-semibold text-gray-700">Food Habit:</label>
          <select name="foodHabit" value={formData.foodHabit} onChange={handleChange} className="w-full border p-2 rounded">
        ...
        {/* Waste */}
        <div>
          <label className="block font-semibold text-gray-700">Waste Generated (kg):</label>
          <input type="number" name="waste" value={formData.waste} onChange={handleChange} className="w-full border p-2 rounded" />
        </div>

        <button onClick={calculateEmissions} className="w-full bg-green-500 text-white py-2 px-4 rounded hover:bg-green-600">
          Calculate Emissions
        </button>

        {emissionResult && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-center text-lg font-semibold text-gray-800 mt-4">
            Total Estimated Emissions: {emissionResult} kg CO₂
          </motion.div>
        )}

        {advice.message.length > 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }} className="mt-6 p-4 bg-green-100 border-l-4 border-green-500 text-green-900 rounded">
            <div className="flex items-center gap-3 mb-2">
              {advice.icon}
              <h2 className="text-lg font-bold">Tips to Reduce Your Emission</h2>
            </div>
